...
Fixpoint pvmul {n: nat} (a b : BasePauliString n) : Scalar * BasePauliString n :=
  (* Looks like dark magic *)
  match a in Vector.t _ n return Vector.t _ n -> Scalar * BasePauliString n  with 
  | ha :: ta => fun b => 
...
Fixpoint pvmul_s {n: nat} (a b : BasePauliString n) : Scalar  :=
    (* Looks like dark magic *)
...
Definition pvmul_alt  {n: nat} (a b : BasePauliString n) : Scalar * BasePauliString n 
...
(* The Pauli String *)
Definition PauliString (n : nat) : Set := Scalar * BasePauliString n.
...
Definition apply_s {n: nat} (s: Scalar) (pstr: PauliString n): PauliString n :=
  let (s', pv) := pstr in
  (s_prod s s', pv).
...
Definition extract_scalar {n: nat} (ps : PauliString n) : Scalar :=
  fst ps.

Definition s_prod_of_pstrings {n: nat} {m: nat} (ps1: PauliString n) (ps2: PauliString m) : Scalar :=
